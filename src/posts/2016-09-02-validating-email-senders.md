---
title: 'Validating email senders'
date: 2016-09-02
---

Email is one of the most heavily used platforms for communication online, and has been for a while.

Most people using email expect that they can see the who sent the email by looking at the sender field in their email client, but in reality the Simple Mail Transfer Protocol (SMTP) that defines how emails are exchanged (as specified in <a href="https://tools.ietf.org/html/rfc5321">rfc 5321</a>) does not provide any mechanism for validating that the sender is actually who he claims to be.
...
Since we're talking about online technologies, get ready for <abbr title="abbreviation">abbr</abbr> galore!

Table of contents:
<ul>
<li><a href="#spf">Sender Policy Framework (SPF)</a></li>
<li><a href="#dkim">DomainKeys Identified Mail (DKIM)</a></li>
<li><a href="#dmarc">Domain Message Authentication Reporting & Conformance (DMARC)</a></li>
</ul>

<h2 id="spf">Sender Policy Framework (SPF)</h2>
...
<pre>v=spf1 <[ip4:<IP range> <SETTING> [ip4:...]] [host:<DOMAIN NAME> <setting> [host:...]]></pre>
For example:
<pre>v=spf1 ip4:192.0.0.0/24 host:example.com ~all</pre>

When an email receiver receives email the process is:
<ol>
<li>Lookup the sending domain's DNS records</li>
<li>Check for an spf record</li>
<li>Compare the spf record with the actual sender</li>
<li>Accept / reject the email based on the output of step 3</li>
</ol>

For more details check out the <a href="http://www.openspf.org/Introduction">SPF introduction</a> or check your domain's setup with the <a href="http://mxtoolbox.com/spf.aspx">SPF checker</a>.

<h2 id="dkim">DomainKeys Identified Mail (DKIM)</h2>
...
For more details, WikiPedia provides a nice <a href="https://en.wikipedia.org/wiki/DomainKeys_Identified_Mail#Overview">DKIM overview</a>.

<h2 id="dmarc">Domain Message Authentication Reporting & Conformance (DMARC)</h2>
...
<pre>v=DMARC1;p=[none|quarantine|reject];rua=[REPORTING EMAIL];ruf=[REPORTING EMAIL]</pre>

For instance

<pre>v=DMARC1;p=none;ruf=email@example.com;rua=email@example.com;</pre>
...
For a bit more detail, check out the <a href="https://dmarc.org/overview/">DMARC overview</a> or check the validity of your current setup using the <a href="https://stopemailfraud.returnpath.com/dmarc/">DMARC validator</a>.